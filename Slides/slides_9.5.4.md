...
Section 9.5.4 — The first natural problems we can prove no algorithm will *ever* solve.
...
Hilbert's 10th problem ($1900$) — and the $70$-year journey to *Matiyasevich's* answer: **no such algorithm exists**.
...
<!--
Hilbert gave his 23 Problems as a lecture at the International Congress of Mathematicians, Paris, 1900 — essentially setting the mathematical agenda for the 20th century in a single morning. The 10th asked for an algorithm to decide integer solvability of Diophantine equations. The resolution unfolded in slow motion over 70 years: Martin Davis (1953) reduced it to a conjecture about Diophantine sets, Julia Robinson and Hilary Putnam extended the work through the 1950s and 60s, and finally Yuri Matiyasevich (then 22, in Leningrad) closed the last gap in 1970 using a specific property of Fibonacci numbers. The final theorem — every recursively enumerable set is Diophantine, known as MRDP — is one of the most surprising results in 20th-century logic. Julia Robinson had worked on it for 20 years; she didn't see the finish line until Matiyasevich's letter arrived.
-->
...
Polynomial equations like $x^2+y^2=z^2$ — easy to write, but no algorithm decides solvability *in general*.
...
Is there *any* TM that always halts and correctly answers "does $M$ accept $w$?" for arbitrary $\langle M,w\rangle$?
...
Turing ($1936$) — at age $23$ — proved that no such algorithm can exist. *Computation has hard limits.*
...
<!--
Turing finished "On Computable Numbers" in May 1936. He was 23. The paper was nearly rejected by the *Proceedings of the London Mathematical Society* — Alonzo Church had just published an equivalent undecidability result using his lambda calculus, and the referees wanted to know whether Turing's paper added anything. Turing's advisor Max Newman intervened, arguing that Turing's machine model was a distinct contribution: concrete, physical-feeling, and (as it turned out) far more intuitive than lambda calculus for reasoning about computation. The paper appeared in late 1936. Turing then went to Princeton and did his PhD with Church. The machine model was effectively unknown to the general public until after Turing's wartime work at Bletchley Park — and that itself stayed classified until the 1970s.
-->
...
Borrow Cantor's trick — *suppose* a decider $H$ exists, then build a machine that contradicts itself.
...
Build $D$ that does the *opposite* of what $H$ predicts $M$ does on its own description $\langle M\rangle$.
...
Feed $D$ its own description: whatever it does, it does the opposite — *both* answers fail.
...
Picture an infinite table — TMs by rows, inputs by columns — and flip the *diagonal* to escape it.
...
"Does $M$ halt on $w$?" — undecidable too, by *reduction* from $A_{\text{TM}}$.
...
Why no debugger can ever truly tell you "your program *will* halt" — for *every* program and input.
...
Undecidability is about *every* input — many specific halting questions are easy to answer.
...
$\Sigma(n)$ — the most squares an $n$-state TM writes before halting — *grows faster than every computable function*.
...
<!--
Tibor Radó introduced the Busy Beaver in 1962 — "On non-computable functions." The values grow astronomically: BB(1)=1, BB(2)=4, BB(3)=6, BB(4)=13. Then things get wild. BB(5) was finally proved exactly equal to 47,176,870 in July 2024, the result of a five-year distributed proof effort called the Busy Beaver Challenge: thousands of candidate 5-state TMs individually classified, by both humans and automated tools, as halting or non-halting. BB(6) is believed to be larger than Graham's number and likely impossible to ever determine — proving a specific 6-state machine halts (or doesn't) can run into statements that are independent of ZFC set theory. Busy Beaver is one of the rare places where the ordinary objects of mathematics (a small machine, a few symbols) bump up against the deepest logical frontiers.
-->
...
If we could compute $\Sigma$, we'd have a *time bound* for every TM — and could decide $A_{\text{TM}}$.
...
$\overline{A_{\text{TM}}}$ is *worse* than undecidable — it isn't even recognizable.
...
A bigger map: *Decidable* $\subset$ *RE* $\subset$ *All*, with co-RE on the other side and a void in between.
...
A different model — a TM that *prints out* the strings of its language, separated by `#`.
...
The two notions coincide — recognizing and enumerating are *the same power* for languages.
...
The trick that makes the proof work — interleave simulations so *no single loop blocks progress*.
...
Phase $i$ runs $M$ on the first $i$ inputs for $i$ steps — every accepting string is *eventually* found.
...
A scoreboard placing each language in the four classes — note **Decidable = RE $\cap$ co-RE**.
...
A $23$-year-old graduate student in $1936$ — *before* electronic computers existed — defined what they could never do.
...
<!--
The full arc: Turing worked at Bletchley Park from 1939 on breaking the German Enigma cipher and later the more sophisticated Lorenz. His Bombe (electromechanical) and Colossus (programmable valve machine, designed by Tommy Flowers based on Turing's ideas) were arguably the first computers — but everything was classified until the 1970s, so Turing's concrete engineering contribution was invisible to his generation. In 1952 Turing was prosecuted under Britain's gross-indecency laws for homosexuality; forced to choose between prison and chemical castration (synthetic estrogen), he chose the latter. He died in 1954 at 41 of cyanide poisoning, widely ruled suicide though the evidence is disputed. He was not pardoned until 2013 (by royal prerogative), and a formal "Alan Turing law" allowing posthumous pardons for others convicted under the same statutes followed in 2017. The ACM's Turing Award, established in 1966, is computer science's highest honor — recognition, really, of how much of the modern field traces back to one 1936 paper by a young graduate student.
-->
...
Hilbert's $1928$ challenge — decide *every* logical formula — answered "no!" by Turing and Church *simultaneously*.
...
<!--
Hilbert's program, launched in the 1920s, was an attempt to put all of mathematics on a foundation that was (1) consistent — no contradictions, (2) complete — every true statement provable, and (3) decidable — an algorithm to settle any well-posed question. Gödel's incompleteness theorems (1931) demolished (1) and (2): any formal system rich enough for arithmetic can neither prove its own consistency nor be both consistent and complete. Five years later, Turing and Church, independently, showed (3) was also unreachable: no general decision algorithm exists. The triple blow ended Hilbert's program as originally conceived. But the techniques developed to prove these limitations — models, proofs, algorithms, formal computation — became the foundation of theoretical computer science. Hilbert died in 1943; he had watched his program collapse and the discipline he helped shape pivot to meet it.
-->
...
Verification, equivalence, malware detection — all run into undecidability; we live with *heuristics*.
...
Why undecidability *matters* — it tells engineers when to stop searching for perfect tools and start approximating.
...
The headline results in one place — undecidability, non-RE, recognizable $=$ enumerable, HALT.
...
Use one undecidable problem to *prove others undecidable* — and watch the impossible spread.
...
Five problems to make these ideas your own — try the diagonal argument on a *different* class.
