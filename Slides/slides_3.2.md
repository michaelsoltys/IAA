...
<!--
The race to multiply faster has spanned 60 years. Toom-Cook (1963) generalized Karatsuba by splitting into $k$ pieces instead of 2, giving $O(n^{1+\epsilon})$ for any $\epsilon > 0$ but with impractical constants. Schönhage and Strassen (1971) used the Fast Fourier Transform to achieve $O(n \log n \log \log n)$, which dominated for decades and is what GMP uses for very large numbers. In 2019, Harvey and van der Hoeven finally proved that multiplication can be done in $O(n \log n)$ — matching the information-theoretic intuition that you should only need to "look at each digit once" (times a log factor for carries). Whether the log factor can be removed remains open.
-->
